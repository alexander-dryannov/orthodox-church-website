...
def _handler_get_table_data(tables):
    ...
    for row in tables.rows[1:]:
        ...
        if date.text != '':
            if len(schedule_list) > 0 and date.text.replace('\n', ' ') == schedule_list[-1]['date']:
                schedule_list[-1]['tt'].append(
                    dict(time=time.text, type_worship=type_worship.text))
            else:
                schedule = dict(
...
def get_data(docx_file):
    ...
    for item in range(len(paragraphs)):
        parsing_data = dict(
            data=_handler_get_table_data(document.tables[item])
        )
